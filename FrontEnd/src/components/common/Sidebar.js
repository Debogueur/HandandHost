...
const Sidebar = ({ activekey }) => {
    ...
    const openChildren1 = useCallback((id) => {
        const otherTabs = document.getElementsByClassName("has-children");
        if (otherTabs) {
            for (let i = 0; i < otherTabs.length; i++) {
                otherTabs[i].className = otherTabs[i].className.replace(" show", "");
    ...
    //     }
    // }
    const GotoChangeMenu = useCallback((val) => {
        if (val === "UI Components") {
            navigate("ui-alerts");
            ...
            <ul className="menu-list flex-grow-1 mt-3">
                {menuData.map((d, i) => {
                    ...
                    if (d.children.length === 0) {
                        return (
                            <li key={`item-${i}`} className="collapsed">
                                <Link
                    ...
                    return (
                        <li key={`parent-${i}`} className="collapsed">
                            <Link
                                className={`m-link ${d.children.filter((child) => "/" + child.routerLink[0] === activekey).length > 0 ? "active" : ""}`}
                                to="#!"
                                onClick={(e) => {
                                    e.preventDefault();
                                ...
                                <ul className="sub-menu collapse has-children" id={`menu-Pages${i}`}>
                                    {d.children.map((data, ind) => {
                                        ...
                                        return (
                                            <li key={`child-${ind}`}>
                                                <Link
                                                    className={activekey === "/" + data.routerLink[0] ? "ms-link active" : "ms-link"}
