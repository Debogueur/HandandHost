   ...
   const tabEvent=(evt, panid, tabClass, navClass)=>{
    ...
    return (
        <div className="border-top mt-5 pt-3">
            ...
            <p>Place any freeform text within a dropdown menu with text and use <Link to="https://v5.getbootstrap.com/docs/5.0/utilities/spacing/">spacing utilities</Link>. Note that you’ll likely need additional sizing styles to constrain the menu width.</p>
            
            <ul className="nav nav-tabs tab-card px-3 border-bottom-0" role="tablist">
                <li className="nav-item"><Link className="nav-link text-nav-link-1 active" to="#!" onClick={(e)=>{e.preventDefault(); tabEvent(e,"text-nav-Preview1","text-tab-pane-1","text-nav-link-1") }}>Preview</Link></li>
                <li className="nav-item"><Link className="nav-link text-nav-link-1" to="#!" onClick={(e)=>{e.preventDefault(); tabEvent(e,"text-nav-Preview2","text-tab-pane-1","text-nav-link-1") }}>HTML</Link></li>
            </ul>
