...
function AccessibilityToast () {
    ...
    return (
        <div className="border-top mt-5 pt-3">
            ...
            <p>Toasts are intended to be small interruptions to your visitors or users, so to help those with screen readers and similar assistive technologies, you should wrap your toasts in an <Link to="https://developer.mozilla.org/en-US/docs/Web/Accessibility/ARIA/ARIA_Live_Regions"><code>aria-live</code> region</Link>. Changes to live regions (such as injecting/updating a toast component) are automatically announced by screen readers without needing to move the user’s focus or otherwise interrupt the user. Additionally, include <code>aria-atomic="true"</code> to ensure that the entire toast is always announced as a single (atomic) unit, rather than announcing what was changed (which could lead to problems if you only update part of the toast’s content, or if displaying the same toast content at a later point in time). If the information needed is important for the process, e.g. for a list of errors in a form, then use the <Link to="https://v5.getbootstrap.com/docs/5.0/components/alerts/">alert component</Link> instead of toast.</p>
            ...
            <p>As the content you’re displaying changes, be sure to update the <Link to="#options"><code>delay</code> timeout</Link> to ensure people have enough time to read the toast.</p>
...
var toastList = toastElList.map(function (toastEl) {
    return new bootstrap.Toast(toastEl, option)
            ...
            <div className="card card-callout p-3 mb-4">
                ...
                <p><Link to="https://v5.getbootstrap.com/docs/5.0/getting-started/javascript/#asynchronous-functions-and-transitions">See our JavaScript documentation for more information</Link>.</p>
            </div>
            <h4 id="show">show</h4>
